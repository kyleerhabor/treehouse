...
(def api "https://api.github.com/graphql")

(defn query [q]
  (:body (http/post api {:body (json/write-str {:query (g/graphql-query q)})
                         ;; The token is actually a personal access token, but :oauth-token produces the correct header.
